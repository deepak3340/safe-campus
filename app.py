...
# ─── AI Disaster Advisor (offline NLP engine) ────────────────────────────────
...
def ai_advisor(message, user_name="there"):
    ...
    if any(g in msg for g in GREETINGS):
        return f"Namaste {user_name}! 🙏 I'm SafeGuard, your AI disaster safety advisor. Ask me anything about earthquake, flood, fire, cyclone safety, first aid, or emergency preparedness. I'm here to help!"

    # Thanks
    if any(t in msg for t in THANKS):
        return "You're welcome! Stay safe and prepared. 🛡️ Remember — preparedness saves lives. Feel free to ask anything else!"

    # Help
    if any(h in msg for h in HELP_WORDS):
        return "I can help you with: 🌍 **Earthquake** safety • 🌊 **Flood** preparedness • 🔥 **Fire** emergency protocols • 🌀 **Cyclone** safety • 🏥 **First Aid** (CPR, bleeding, burns, fractures) • 🎒 **Emergency kit** checklist • 📞 **NDMA helplines** and contacts. Just ask naturally — like 'What do I do during an earthquake?' or 'How do I make an emergency kit?'"
    ...
    if best_match and best_score > 0:
        ...
        # Find best sub-topic
        sub_scores = {}
        for sub, content in data.items():
            if sub == "keywords": continue
            keywords_in_sub = re.findall(r'\w+', sub)
            sub_scores[sub] = sum(1 for kw in keywords_in_sub + ["before","during","after","what","how","kit","india","cpr","bleeding","burn","choking","fracture"] if kw in msg)

        # Also check message words directly
        if "before" in msg or "prepare" in msg or "prevention" in msg:
            preferred = "before"
        elif "during" in msg or "when it happens" in msg or "right now" in msg:
            preferred = "during"
        elif "after" in msg or "post" in msg or "recover" in msg:
            preferred = "after"
        elif "india" in msg or "ndma" in msg or "zone" in msg:
            preferred = "india"
        elif "kit" in msg or "bag" in msg or "pack" in msg or "prepare" in msg:
            preferred = "kit" if "kit" in data else "before"
        elif "cpr" in msg or "cardiac" in msg or "heart" in msg:
            preferred = "cpr"
        elif "bleed" in msg or "wound" in msg or "cut" in msg:
            preferred = "bleeding"
        elif "burn" in msg:
            preferred = "burns"
        elif "chok" in msg or "throat" in msg:
            preferred = "choking"
        elif "fractur" in msg or "bone" in msg or "broken" in msg:
            preferred = "fracture"
        elif "extinguish" in msg or "pass" in msg or "foam" in msg:
            preferred = "extinguisher"
        elif "contact" in msg or "number" in msg or "call" in msg or "helpline" in msg:
            preferred = "india" if "india" in data else "info"
        else:
            preferred = list(data.keys())[1] if len(data) > 1 else None  # first non-keywords sub
        ...
        if content_key and content_key in data:
            ...
            return f"{icon} **{category.replace('_',' ').title()} — {content_key.title()}**\n\n{data[content_key]}\n\n💡 *Ask me more: 'What to do before/during/after {category}?' or 'Show me the emergency kit list'*"

    # Specific number questions
    if re.search(r'(number|call|helpline|phone)', msg):
        return KNOWLEDGE_BASE["ndma"]["info"]

    # Fallback
    fallback_tips = [
        "I didn't quite catch that — try asking something like: 'What should I do during an earthquake?' or 'How do I perform CPR?' or 'What goes in an emergency kit?'",
        "Could you rephrase that? I can help with earthquake, flood, fire, cyclone safety, first aid, and emergency preparedness!",
        "I'm here to help with disaster safety. Try: 'Earthquake safety tips', 'First aid for bleeding', or 'India cyclone helplines'."
    ]
